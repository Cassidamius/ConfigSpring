...
<form id="searchForm" method="post"
	action="${pageContext.request.contextPath}/findResourceList">
	<table>
		<tr>
			<td>资源名：</td>
			...
			<td>资源：</td>
			<td><input type="text" id="rescString" name="rescString"
				value="${resource.rescString}" /></td>
		</tr>
		<tr>
			<td colspan="4"><input type="button" value="查询" name="search"
				onclick="searchList();" />&nbsp;&nbsp; <input type="button"
				value="清空" onclick="clean();" /> &nbsp;&nbsp; <input type="button"
				value="添加"
				onclick="toAddPage('${pageContext.request.contextPath}/toAddResourcePage');" />
			</td>
		</tr>
	</table>
...
<div
	...
	<c:if test="${pageResultSet.list != null}">
		<table border="1">
			...
			<c:forEach var="item" items="${pageResultSet.list}">
				<tr>
					<td><a
						href="${pageContext.request.contextPath}/toEditResourcePage/${item.id}"
						class="button gray">修改</a>&nbsp;&nbsp; <a
						href="javascript:void(0);"
						onclick="deleteItem('${pageContext.request.contextPath}/deleteResource', '${item.id}');"
						class="button gray">删除</td>
					<td>${item.name}</td>
					<td>${item.rescType}</td>
