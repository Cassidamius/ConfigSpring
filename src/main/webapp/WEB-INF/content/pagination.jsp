...
共${pageResultSet.pageInfo.totalRow}条，共${pageResultSet.pageInfo.totalPage}页；当前第${pageResultSet.pageInfo.currentPage}页；每页显示：
	...
	<c:if test="${pageResultSet.pageInfo.currentPage == 1 && pageResultSet.pageInfo.totalPage > 1}">首页上一页<a href="javascript:changePage(${pageResultSet.pageInfo.currentPage + 1},${pageResultSet.pageInfo.totalRow});">下一页</a><a href="">末页</a></c:if>
	<c:if test="${pageResultSet.pageInfo.currentPage > 1 && pageResultSet.pageInfo.totalPage > pageResultSet.pageInfo.currentPage}"><a href="">首页</a><a href="">上一页</a><a href="">下一页</a><a href="">末页</a></c:if>
	<c:if test="${pageResultSet.pageInfo.currentPage > 1 && pageResultSet.pageInfo.totalPage == pageResultSet.pageInfo.currentPage}"><a href="">首页</a><a href="">上一页</a>下一页末页</c:if>